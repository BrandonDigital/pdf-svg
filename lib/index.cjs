...
var PDFWriter = class {
  ...
  labToRgb(L, a, b) {
    ...
    bl = bl > 31308e-7 ? 1.055 * Math.pow(bl, 1 / 2.4) - 0.055 : 12.92 * bl;
    return {
  ...
  generatePDF(doc) {
    ...
    const compressedContent = import_pako.default.deflate(contentStream);
    const resources = {};
    ...
    for (const [name, cs] of Object.entries(colorSpaces)) {
      if (cs.type === "Separation") {
        ...
        if (cs.name) {
          ...
          this.objects[labRef] = "[/Lab<</BlackPoint[0.0 0.0 0.0]/Range[-128.0 127.0 -128.0 127.0]/WhitePoint[0.964203 1.0 0.824905]>>]";
          ...
          const labArray = `[/Lab << /WhitePoint [${cs.whitePoint.map((n) => this.formatNumber(n)).join(" ")}] /Range [${cs.range.map((n) => this.formatNumber(n)).join(" ")}] >>]`;
  ...
  generateXMPMetadata(plateNames) {
    ...
    if (plateNames.length > 0) {
      plateNamesXML = `
         <xmpTPg:PlateNames>
            <rdf:Seq>
${plateNames.map((name) => `               <rdf:li>${name}</rdf:li>`).join("\n")}
            </rdf:Seq>
         </xmpTPg:PlateNames>`;
    }
...
var SVGRenderer = class {
  ...
  applyColor(color, type) {
    if (this.options.colorCallback) {
      ...
      if (result) {
        if (typeof result === "object" && result.c !== void 0) {
          if (type === "fill") {
...
var PDFDocument = class {
  ...
  _ensureLabColorSpace() {
    ...
    const colorSpaceName = `LAB${Object.keys(this.resources.ColorSpace).length + 1}`;
    this.resources.ColorSpace[colorSpaceName] = {
      type: "Lab",
      ...
      case "cmyk":
        this.contentStream.push(
          `${this._formatNumber(color.c)} ${this._formatNumber(
            color.m
          )} ${this._formatNumber(color.y)} ${this._formatNumber(color.k)} ${type === "fill" ? "k" : "K"}`
        );
        break;
      case "lab":
        ...
        this.contentStream.push(
          ...
          `${this._formatNumber(color.L / 100)} ${this._formatNumber(
            (color.a + 128) / 255
          )} ${this._formatNumber((color.b + 128) / 255)} ${type === "fill" ? "scn" : "SCN"}`
        );
        break;
      case "spot":
